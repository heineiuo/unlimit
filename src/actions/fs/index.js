export default {
  queryFileContent: require('./queryFileContent').default,
  queryOneByFullPath: require('./queryOneByFullPath').default,
  queryOneById: require('./queryOneById').default,
  queryFile: require('./queryFile').default,
  mutateInsertOne: require('./mutateInsertOne').default,
  mutateUpload: require('./mutateUpload').default,
  mutateDelete: require('./mutateDelete').default,
  mutateFileContent: require('./mutateFileContent').default,
}
